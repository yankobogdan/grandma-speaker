...
#error setCACert not implemented
                ...
                // NOTE: this library version's public API (beginSslWithBundle) never accepted a
                // bundle length, so there is no real size to pass here. Not used by GeminiLive
                // (which calls beginSslWithCA instead) - patched only to satisfy the current
                // ESP32 core's NetworkClientSecure::setCACertBundle(ptr, size) signature.
                _client.ssl->setCACertBundle(_CA_bundle, 0);
#endif
#if defined(ESP32)
...
#if defined(ESP32)
            if (_client.isSSL && _client.ssl) {
                char errBuf[160];
                _client.ssl->lastError(errBuf, sizeof(errBuf));
                DEBUG_WEBSOCKETS("[WS-Client] TLS lastError: %s\n", errBuf);
            }
#endif
        }
    } else {
