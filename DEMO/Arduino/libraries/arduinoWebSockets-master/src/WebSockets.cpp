 ...
 */
// Last close frame received from the server. Exposed so the application can
// distinguish a normal drop from a refusal (e.g. the Live API's quota error),
// which is otherwise only visible as an unexplained reconnect loop.
uint16_t ws_last_close_code = 0;
char ws_last_close_reason[192] = {0};
    ...
    if(ok) {
        ...
        switch(header->opCode) {
            ...
            case WSop_close: {
                ...
                // Always surfaced (not just under DEBUG_WEBSOCKETS): when the
                // Live API rejects a setup message this close reason is the only
                // explanation we get, and it's otherwise invisible. Stashed in
                // globals so the application layer can react (see GeminiLive).
                ws_last_close_code = reasonCode;
                ws_last_close_reason[0] = '\0';
                if(header->payloadLen > 2) {
                    size_t rl = header->payloadLen - 2;
                    if(rl >= sizeof(ws_last_close_reason)) rl = sizeof(ws_last_close_reason) - 1;
                    memcpy(ws_last_close_reason, payload + 2, rl);
                    ws_last_close_reason[rl] = '\0';
                    Serial.printf("[WS] server closed, code %u: %s\n", reasonCode, ws_last_close_reason);
                } else {
                    Serial.printf("[WS] server closed, code %u\n", reasonCode);
                }
