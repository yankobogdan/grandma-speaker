...
#include <stdint.h>
extern uint16_t ws_last_close_code;
extern char ws_last_close_reason[192];

// To re-enable arduinoWebSockets' verbose internal logging while diagnosing a
// connection issue, uncomment this (it must reach every translation unit that
// compiles this library, not just ones that happen to include it after GeminiLive.h,
// which is why it's forced here rather than left to includer-order).
...
// Raised from the library's default 15KB: Gemini Live's audio-delta frames
// routinely exceed that, and this check disconnects (WS close code 1009,
// "message too big") *before* the frame ever reaches the app's event callback
// - invisible from our side except as an unexplained mid-reply disconnect.
// This is a transient per-frame malloc, not a static buffer, so raising it
// only costs memory while a large frame is actually being processed.
